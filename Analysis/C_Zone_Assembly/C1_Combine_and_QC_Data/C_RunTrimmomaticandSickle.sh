...
for z in ../0_*/zone.[A-Z]; do
    if [ -d trimmomatic/${z#../0_*/} ]; then
        echo "directory exists"
    else
        mkdir ./trimmomatic/${z#../0_*/}
    fi

    if [ -d sickle/${z#../0_*/} ]; then
        echo "directory exists"
    else
        mkdir ./sickle/${z#../0_*/}
    fi
...
#in each zone
#for f in *S*_L001*1.u.t*; do cat ${f%1.u.*}*u* > ${f%1.u.*}unpaired.trimclean.sickleclean.fq.gz; done
